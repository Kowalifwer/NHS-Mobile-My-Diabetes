    ...
    return (
        <View style={GlobalStyle.BodyGeneral}>
            {(object_is_empty(food_input_components_data[id]["scanned_item_object"])) ? 
                <Text style={[GlobalStyle.CustomFont, GlobalStyle.Cyan, {marginTop:30}]}>Diary data for food item {id+1}. </Text> : 
                <Text style={[GlobalStyle.CustomFont, GlobalStyle.Cyan]}>Some data for food item {id+1} has been filled by Barcode scanner! Please double check and make sure to update anything that is not correct!</Text>
            }
